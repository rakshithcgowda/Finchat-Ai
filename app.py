...
GOOGLE_API_KEY = os.environ.get(
    "GOOGLE_API_KEY",
    "AIzaSyANbVVzZACnYnus00xwwRRE01n34yoAmcU"
)
...
def init_db(db_path: str = "users.db"):
    try:
        ...
        for col, col_type in [("location_id", "TEXT"), ("last_login", "TIMESTAMP"), ("created_at", "TIMESTAMP")]:
...
def create_default_admin(conn):
    try:
        ...
        logging.error(f"Failed to create default admin: {e}", exc_info=True)
...
def verify_password(hashed: bytes, password: str) -> bool:
    try:
        ...
        logging.error(f"Password verification failed: {e}", exc_info=True)
        return False
...
def call_mistral(messages: List[Dict[str, str]], temperature: float = 0.2, top_p: float = 1.0,
                max_tokens: int = 1024, stop: List[str] = None, stream: bool = False,
                user: str = None, logit_bias: Dict[int, float] = None) -> str:
    payload = {
...
def call_deepseek(messages: List[Dict[str, str]], model: str = "deepseek-chat",
                 temperature: float = 0.7, max_tokens: int = 2000, top_p: float = 1.0,
                 frequency_penalty: float = 0.0, presence_penalty: float = 0.0, stream: bool = False) -> str:
    try:
        ...
        logging.error(f"DeepSeek API error: {e}", exc_info=True)
        return f"Error: {e}"

def save_interaction(conn, feature: str, input_text: str, output_text: str):
    if not conn:
        logging.error("No database connection for saving interaction")
        return
    try:
        if st.session_state.get("username"):
            ...
            conn.commit()
            logging.info(f"Interaction saved for user: {st.session_state.username}, feature: {feature}")
    except sqlite3.OperationalError as e:
        logging.error(f"Failed to save interaction: {e}", exc_info=True)

# ─── Lease Summarization UI ────────────────────────────────────────────────────
def lease_summarization_ui(conn):
    ...
    st.markdown("Upload your lease PDF and get a **fast** and concise summary—either the full document or page-by-page.")
    uploaded_file = st.file_uploader("Upload Lease Document (PDF)", type=["pdf"], key="lease_file_uploader")
    ...
    ai_engine = st.radio("Select AI Model", ["in-depth"], index=0, horizontal=True, key="lease_ai_engine")
    summary_mode = st.radio("Summary Mode", ["Full Document", "Page-by-Page"], index=1, horizontal=True, key="lease_summary_mode")
    ...
    if 'last_summary' in st.session_state and st.session_state.get('last_file') == uploaded_file.name:
        ...
        st.markdown("Upload a Word document (.docx) with any content related to the lease summary.")
        docx_file = st.file_uploader("Upload Document (Word Document)", type=["docx"], key="lease_docx_uploader")
        if docx_file:
            try:
                ...
                logging.error(f"Failed to process Word document: {e}", exc_info=True)
...
def deal_structuring_ui(conn):
    ...
    if st.button("Generate Strategies", type="primary", key="gen_strat"):
        ...
        with st.spinner("Developing strategies..."):
            strategies = call_mistral(
                messages=[
                    {"role": "system", "content": "You are a real estate investment strategist."},
                    {"role": "user", "content": prompt}
                ]
            )
        ...
        matches = re.findall(r"Strategy\s+(\d+):\s*(.*?)(?=(?:Strategy\s+\d+:)|\Z)", strategies, flags=re.S)
        if matches:
...
def build_guided_prompt(details: dict, detail_level: str) -> str:
    ...
    sections = [
        ...
        "Format the output as an empathetic offer letter."
    ]
    ...
    if st.session_state.offer_stage == 'input_method':
        ...
        if method == 'Guided Form':
            with st.form("offer_details_form"):
                ...
                local_experience = st.text_area("Local Investment Experience*", placeholder="e.g., I have been investing in [Local Area] for several years.", key="local_experience")
                ...
                profit_reason = st.text_area("Reason for Profit*", value="I maintain at least a 15% profit margin to protect against unforeseen risks.", key="profit_reason")
    ...
    if st.session_state.offer_stage == 'offer_generation':
        ...
        with st.spinner("Generating..."):
            try:
                ...
                logging.error(f"Offer generation failed: {e}", exc_info=True)
                st.error(f"Offer generation failed: {e}")
                return
...
def admin_portal_ui(conn):
    ...
    with tab1:
        ...
        try:
            ...
            logging.error(f"User management query failed: {e}", exc_info=True)
    ...
    with tab2:
        ...
        try:
            ...
            logging.error(f"Subscription management failed: {e}", exc_info=True)
            st.error(f"Failed to manage subscriptions: {e}")
        ...
        with st.expander("Upload Training Materials"):
            ...
            if uploaded and st.button("Upload"):
                try:
                    ...
                    logging.error(f"Content upload failed: {e}", exc_info=True)
                    st.error(f"Content upload failed: {e}")
...
def history_ui(conn):
    ...
    try:
        ...
        logging.error(f"History query failed: {e}", exc_info=True)
...
def chatbot_ui(conn):
    ...
    if user_input:
        ...
        try:
            ...
            logging.error(f"Chatbot query failed: {e}", exc_info=True)
            st.error(f"Chatbot error: {e}")

# ─── OCR PDF UI ───────────────────────────────────────────────────────────────
def ocr_pdf_ui(conn):
    st.header("🔍 OCR PDF Converter")
    st.markdown("Convert scanned/non-selectable PDFs into searchable PDF documents.")
    with st.expander("⚙️ OCR Configuration", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            ocr_engine = st.radio("OCR Engine", ["Tesseract"], index=0)
    ...
    if st.button("Convert to Searchable PDF"):
        ...
        if not force_ocr:
            try:
                ...
                logging.error(f"Pre-OCR text check failed: {e}", exc_info=True)
        ...
        try:
            ...
            logging.error(f"OCR conversion failed: {e}", exc_info=True)
        ...
        if submitted:
            ...
            try:
                ...
                if user and verify_password(user[0], password):
                    ...
                    st.session_state.location_id = user[2] if len(user) > 2 else None
                    cursor.execute(
                ...
                logging.error(f"Login query failed: {e}", exc_info=True)
...
def main():
    ...
    try:
        ...
        if not sub:
            conn.execute(
                "INSERT INTO subscriptions (username, lease_analysis, deal_structuring, offer_generator) VALUES (?, 0, 0, 0)",
                (st.session_state.username,)
            )
            conn.commit()
            sub = (0, 0, 0)
        ...
        logging.error(f"Failed to fetch subscription data: {e}", exc_info=True)
        st.error(f"Failed to fetch subscription data: {e}")
        return
